...
export default function MissingTranslations({ pageContext }) {
  ...
  const nonEnglishLocales = new FunctionalSet(pageContext.allLocales).filter(locale => locale != 'en');
  ...
  const keysWithMissingTranslations = (
    allKeys.filter(
      key => nonEnglishLocales.some(
        locale => !klft[key][locale]
      )
    )
  );
  ...
  const keysWithFileDisrepencies = (
    allKeys.filter((key) => {
      // e.g. [ 'es', 'fr', 'jp']
  ...
  return (
    <div>
            ...
            <div
              ...
              style={{ 
                background: `
                  ...
                  conic-gradient(${{es: 'rgb(202 138 4)', fr: 'rgb(37 99 235)', ja: 'rgb(22 163 74)', en: 'rgb(220 38 38)'}[locale]} ${Math.round(100 * translatedKeyCountByLocale[locale] / allKeys.size)}%, rgb(209 213 219) 0)
                `
              }}
            >
              <div>{locale}</div>
            </div>
            <div className="text-xl"><span className={{es: 'text-yellow-600', fr: 'text-blue-600', ja: 'text-green-600', en: 'text-red-600'}[locale]}>{translatedKeyCountByLocale[locale]}</span> / {allKeys.size}</div>
      ...
      <h2>Disrepencies</h2>
      Found <strong>{keysWithFileDisrepencies.size}</strong> disrepencies.{' '}
      These are cases where a key is translated in every locale,{' '}
      but the files where the translations appear differ by locale.
        ...
        {keysWithFileDisrepencies.map((key) => (
          <div key={key} className="p-4 flex items-center justify-center">
            <div>
              <table>
                <tr>
                  ...
                  {allLocales.map((locale) => (
                    <th key={locale} className="p-2 border-1 bg-gray-100">{locale}</th>
                  ))}
                </tr>
                {keyFilesMap[key].map((file) => (
                  <tr key={file}>
                    ...
                    {allLocales.map((locale) => {
                      ...
                      return (
                        <td key={locale} className={`border-1 p-2 text-center ${translation ? 'bg-green-300' : 'bg-white'}`} title={translation}>
                          {translation ? 'x' : ''}
                        </td>
                      )
                    })}
              ...
              <div className="mt-4 text-center"><code>"{key}"</code></div>
            </div>
          </div>
        ))}
      </div>
      ...
      {Object.keys(localeSlashFileToMissingKeysMap).sort().map((localeSlashFile) => (
        <tr key={localeSlashFile}>
          <th className={`border-1 p-2 w-[200px] ${ bgByLocale[localeSlashFile.replace(/\/.*/, '')] }`}>
            <Copyable text={localeSlashFile} />
          </th>
          <td className="border-1 p-2">
            <details>
              <summary>
                <strong>{localeSlashFileToMissingKeysMap[localeSlashFile].size}</strong> missing translations.
              </summary>
              <pre className="bg-inherit my-4 whitespace-pre-wrap">
                <Copyable fullButton={true} text={localeSlashFileToMissingKeysMap[localeSlashFile].reduce(
      ...
      {keysWithMissingTranslations.map((key) => (
        <table key={key} className="my-8 w-full">
          ...
          {
            allLocales.filter((locale) => klft[key][locale] || locale != 'en').map((locale) => (
              keyFilesMap[key].map((file) => (
                <tr key={locale}>
  ...
  return (
    <button className="text-left" onClick={() => {
      navigator.clipboard.writeText(text).then(
        ...
        (error) => { addToast({message: `Could not copy text`, severity: SEVERITY.danger, error }) }
      );
    }}>
      {text}
      ...
      {
        fullButton
        ? <span className="tw-btn tw-btn-primary mt-4">Copy <FontAwesomeIcon icon={faCopy} className="ml-1"/></span>
