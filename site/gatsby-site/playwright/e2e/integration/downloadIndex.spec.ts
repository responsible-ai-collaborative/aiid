...
test.describe('Download Algolia Index', () => {
  const url = '/downloadIndex';
  const downloadsFolder = process.env.PLAYWRIGHT_DOWNLOADS_FOLDER || './downloads';

  test('Should download a properly constructed Algolia index', async ({ page }) => {
    await page.goto(url);
    ...
    const filePath = path.join(downloadsFolder, 'index.json');
    const index = JSON.parse(await page.waitForEvent('download').then((download) => download.path()));

    expect(index).toHaveLength(reports.length);
    ...
    expect(report).toMatchObject({
      ...
      authors: ['Rob Price'],
      image_url: 'https://amp.businessinsider.com/images/56f3ebc19105842b008b870b-960-480.png',
      cloudinary_id: 'reports/amp.businessinsider.com/images/56f3ebc19105842b008b870b-960-480.png',
      language: 'en',
    ...
    expect(report.classifications).toHaveLength(testClassifications.length);

    for (const classification of report.classifications) {
      expect(testClassifications).toContain(classification);
    }
  });
});
