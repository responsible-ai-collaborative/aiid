...
export async function deleteSubmittedDocument(deleteDocumentData, callback) {
  ...
  await user.functions.deleteSubmittedDocument(deleteDocumentData);
...
export async function promoteReport(newReportData, callback) {
  ...
  await user.functions.promoteReport(newReportData);
...
async function createReportForReview(newReportData, callback) {
  ...
  console.log(newReportData);
  await user.functions.createReportForReview(newReportData);
...
async function quickAddReport(newReportData, callback) {
  ...
  console.log(newReportData);
  await user.functions.quickAdd(newReportData);
...
class API extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.newIncidentData !== prevProps.newIncidentData &&
      this.props.newIncidentData !== {}
    ) {
  ...
  componentDidMount() {
    ...
    getUser(() => {
      this.setState({ loadingMessage: connectedMessage });
    });
  }
