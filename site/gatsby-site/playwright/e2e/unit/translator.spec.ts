...
test('Translations - Should translate languages only if report language differs from target language', async ({ page }) => {
  ...
  const reportsCollection = {
    find: () => ({
      toArray: async () => reports,
    }),
  };

  const reportsENCollection = {
    find: () => ({
      toArray: async () => translatedReportsEN,
    }),
  ...
  const reportsESCollection = {
    find: () => ({
      toArray: async () => translatedReportsES,
    }),
    insertMany: async () => ({ insertedCount: 1 }),
  };
  ...
  const translateClient = {
    translate: (payload: any[], { to }: { to: string }) => [payload.map((p) => `test-${to}-${p}`)],
  };
  ...
  // Add your assertions here based on your test framework's syntax
  console.log('Test completed for translation with different target languages');
});
...
test("Translations - Shouldn't call Google's translate api if dryRun is true", async ({ page }) => {
  ...
  const translateClient = {
    translate: (payload: any[], { to }: { to: string }) => [payload.map((p) => `translated-${to}-${p}`)],
  };
  ...
  // Add your assertions here based on your test framework's syntax
  console.log('Test completed for dryRun mode');
});
