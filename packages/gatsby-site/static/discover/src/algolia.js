...
index.searchClient = algoliasearch('8TNY3YFAO8', '55efba4929953a53eb357824297afb4c');
  ...
  const renderRefinementList = (renderOptions, isFirstRender) => {
    ...
    widgetParams.container.querySelector('ul').innerHTML = items
      .map(
        (item) => `
            <button
              class="d-flex justify-content-between align-items-center btn btn-sm btn-block ${
                item.isRefined ? 'btn-primary' : 'btn-outline-primary'
              } btn-no-margin"
              href="${createURL(item.value)}"
              data-value="${item.value}"
            >
              ${item.label} <span class="badge ${
          item.isRefined ? 'badge-secondary' : 'badge-secondary'
        } badge-pill"> ${item.count}</span>
            </button>
        `
      )
      .join('');
    if (items.length < 1) {
      widgetParams.container.querySelector('ul').innerHTML =
        "<div class='d-flex justify-content-center'>no results</div>";
    }
...
const customRefinementList = function (config) {
  return instantsearch.connectors.connectRefinementList(
    renderRefinementWithAttribute(config.attribute)
  )(config);
};
search.addWidgets([
...
const renderHits = (renderOptions, isFirstRender) => {
  ...
  hits.forEach(function (element) {
    ...
    if (
      typeof element._snippetResult.text !== 'undefined' &&
      element._snippetResult.text.matchLevel === 'full'
    ) {
      element.for_render +=
        "</p><blockquote class='blockquote'><p>&hellip;" +
        element._snippetResult.text.value +
        '&hellip;</blockquote>';
    }
    element.for_render += '</p>';
    if (typeof element.text === 'string') {
      ...
      if (renderBig) {
        element.for_render +=
          "<div class='card_full_text'><p>" + element.text.replace(/\n/g, '</p><p>') + '</p></div>';
      } else {
        element.for_render +=
          "<div class='card_short_text'><p>" + shortText.replace(/\n/g, '</p><p>') + '</p></div>';
      }
    }
          ...
          <div class="card_detail card_pad ${cardCSS}">
            <div class="card">
              <div class="card-header">
                <h1 class="article-header">${instantsearch.highlight({
                  attribute: 'title',
                  hit: item,
                })}</h1>
                <p class="subhead">${item.source_domain} &middot; ${
          item.date_published ? item.date_published.substring(0, 4) : 'Needs publish date'
        }${renderBig ? ' &middot; ' + item.authors.join(', ') : ''}</p>
              </div>
              <div class="card-body">
                <article>
                  ${item.for_render}
              ...
              <div class="align-bottom">
                <p><img class="image-preview" onerror="this.style.display='none';this.onerror=null;this.parentElement.className='fas fa-robot fa-5x';" src='${
                  item.image_url_hash
                }'></p>
                <button style="display:${
                  renderBig ? 'none' : 'inherit'
                }" type="button" class="btn btn-secondary btn-sm btn-block assignment-button" data-incident-id="${
          item.incident_id
        }" onclick="index.filterToIncident(${item.incident_id})">Show Details on Incident #${
          item.incident_id
        }</button>
                <button style="display:${
                  api.isAdmin ? 'inherit' : 'none'
                }" type="button" class="btn btn-danger btn-sm btn-block" onclick="api.deleteReport(${
          item.ref_number
        }, ${item.report_number})">Delete Ref ${item.ref_number}, Rep #${
          item.report_number
        }</button>
              </div>
              <div class="card-footer text-muted">
                <p>
                  ...
                  <i class="pointer far fa-id-card"  title="Authors" onclick="showAuthorModal(this)" data-toggle="modal" data-target="#authormodal"><span style="display:none;">${
                    item.authors
                  }</span></i>
                  <i class="pointer fas fa-user-shield"  title="Submitters" onclick="showSubmitterModal(this)" data-toggle="modal" data-target="#submittermodal"><span style="display:none;">${
                    item.submitters
                  }</span></i>
                  ${
                    typeof api !== 'undefined' && api.isAdmin
                      ? `<i class="pointer fas fa-search-plus" title="Complete Information" onclick="showDetailModal(this)" data-target="#detailmodal" data-detail-number="${item.__position}" data-ref-number="${item.ref_number}" data-incident-id="${item.incident_id}" data-report-number=${item.report_number}></i>`
                      : ''
                  }
                  <i class="pointer far fa-flag" title="Flag Report" onclick="showFlagModal(this)" data-toggle="modal" data-target="#flagmodal" data-incident-id="${
                    item.incident_id
                  }" data-report-number="${item.report_number}"></i>
                  <span class="pointer" data-incident-id="${
                    item.incident_id
                  }" onclick="index.filterToIncident(${
          item.incident_id
        })"><i class="fas fa-hashtag" title="Incident ID" ></i>${item.incident_id}</span>
                </p>
              </div>
            </div>
          </div>`
