...
urlsToTest.forEach(({ page, url, title, shareButtonSections }) => {
  ...
  test(`${page} page should have a Twitter share button`, async ({ page }) => {
    ...
    await expect(page.url()).toContain(`https://x.com/intent/post?text=${encodeURIComponent(title).replace(/%20/g, '+')}&url=${encodeURIComponent(canonicalUrl)}`);
  });
  ...
  test(`${page} page should have a LinkedIn share button`, async ({ page }) => {
    ...
    await page.waitForTimeout(8000);
    await expect(page.url()).toContain('https://www.linkedin.com/');
  });
  ...
  test(`${page} page should have an Email share button`, async ({ page }) => {
    ...
    await page.waitForTimeout(8000); // Wait to ensure the action is triggered
    await expect(page.url()).toBe(canonicalUrl);
  ...
  test(`${page} page should have a Facebook share button`, async ({ page }) => {
    ...
    await page.waitForTimeout(8000);
    await expect(page.url()).toMatch(/https:\/\/www\.facebook\.com\/(login\.php|sharer\/sharer\.php\?u=)/);
  });
});
