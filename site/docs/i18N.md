...
```
GATSBY_AVAILABLE_LANGUAGES=en,es,fr,ja,pt
```

> **Note on `pt` (Brazilian Portuguese):** Portuguese is registered as a **UI-only** locale. We do not run the report/incident machine-translation pipeline for it, and we do not maintain translated MDX/Prismic blog content. Dynamic content (reports, incidents, blog posts) under `/pt/` routes falls back to English by design.
...
GATSBY_AVAILABLE_LANGUAGES=en,es,fr,ja,pt
```
Also update the `GATSBY_AVAILABLE_LANGUAGES` repo variable in **GitHub → Settings → Secrets and variables → Actions → Variables** so CI/CD picks up the new locale, and set it on Netlify (either via the deploy workflow or directly in the Netlify env-var UI).
