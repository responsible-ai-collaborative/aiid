...
test.describe('Cite pages', () => {
    ...
    test('Should show proper entities card text', async ({ page, skipOnEmptyEnvironment }) => {
        await page.goto('/cite/3/');
    ...
    test.skip('There should not be image errors (400)', async ({ page, skipOnEmptyEnvironment }) => {
        page.on('console', (msg) => {
            if (msg.type() === 'error') {
