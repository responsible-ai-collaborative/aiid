...
describe('Cite pages', () => {
  ...
  it('Should display similar incidents', () => {
    ...
    cy.waitForStableDOM();
    cy.get('body').then((body) => {
      if (!body.text().includes('Incident 9 not found')) {
  ...
  maybeIt('Should subscribe to incident updates (user authenticated)', () => {
    ...
    cy.waitForStableDOM();
    cy.get('body').then((body) => {
      if (!body.text().includes('Incident 51 not found')) {
        cy.conditionalIntercept(
          '**/graphql',
  ...
  it('Should show proper entities card text', () => {
    ...
    cy.waitForStableDOM();
    cy.get('body').then((body) => {
      if (!body.text().includes('Incident 67 not found')) {
    ...
    cy.waitForStableDOM();
    cy.get('body').then((body) => {
      if (!body.text().includes('Incident 72 not found')) {
        cy.get('[data-cy="alleged-entities"]').should(
          'have.text',
    ...
    cy.waitForStableDOM();
    cy.get('body').then((body) => {
      if (!body.text().includes('Incident 30 not found')) {
        cy.get('[data-cy="alleged-entities"]').should(
          'have.text',
