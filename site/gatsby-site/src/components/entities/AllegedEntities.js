...
function EntityBadge({ entity }) {
  ...
  return (
    <Link
      to={to}
      className="hover:no-underline inline-block bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 my-0.5 rounded dark:bg-blue-200 dark:text-blue-800"
    >
      {entity.name}
    </Link>
  );
...
function PartyBadge({ entity }) {
  ...
  return (
    <Link
      to={to}
      className="hover:no-underline inline-block bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-0.5 my-0.5 rounded dark:bg-green-200 dark:text-green-800"
    >
      {entity.name}
    </Link>
  );
...
function ImplicatedSystemsList({ entities }) {
  return (
    <div className="mt-2">
      {entities.length > 0 && (
        <>
          <Trans ns="entities">AI implicated system{entities.length > 1 ? 's' : ''}:</Trans>{' '}
