  ...
  return (
    <>
      <div
        className={`${
          expanded ? '' : 'flex'
        } items-center bg-white rounded-lg border  shadow-md dark:border-gray-700 dark:bg-gray-800 ${className}}`}
      ...
      >
        ...
        {expanded ? (
          <p className={`text-justify m-2`}>{cardText}</p>
        ) : (
          <div className="m-2 w-2/3">{cardText}</div>
        )}
      </div>
    </>
  );
